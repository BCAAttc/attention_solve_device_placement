...
class StateTSP(NamedTuple):
    ...
    def initialize(loc, visited_dtype=torch.uint8):
        ...
        return StateTSP(
            loc=loc,      # loc  =  input [1024，20,20]
            # dist=(loc[:, :, None, :] - loc[:, None, :, :]).norm(p=2, dim=-1), # 每个点到其他点的距离 【1024，20，20】
            dist=loc, # 每个点到其他点的距离 【1024，20，20】
            # dist=(loc[:, :, None, :] - loc[:, None, :, :]).norm(p=2, dim=-1), # 每个点到其他点的距离
            ids=torch.arange(batch_size, dtype=torch.int64, device=loc.device)[:, None],  # Add steps dimension 【1024，1】
            first_a=prev_a,
            prev_a=prev_a,
    ...
    def update(self, selected):
        ...
        # if i==0:
        #     C = self._replace(first_a=first_a, prev_a=prev_a, visited_=visited_,
        #                   lengths=lengths, cur_coord=cur_coord, i=self.i + 1)
        # else:
        #     C = C._replace(first_a=first_a, prev_a=prev_a, visited_=visited_,
        #                   lengths=lengths, cur_coord=cur_coord, i=self.i + 1)
        # i+=1
        # return C
